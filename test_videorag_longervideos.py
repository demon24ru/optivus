...
if __name__ == '__main__':
    ...
    ## learn
    video_base_path = f'./test_longervideos/{sub_category}/videos/'
    video_files = sorted(os.listdir(video_base_path))
    video_paths = [os.path.join(video_base_path, f) for f in video_files]
    videorag = VideoRAG(llm=longervideos_llm_config, working_dir=f"./test_longervideos/videorag-workdir/{sub_category}")
    videorag.insert_video(video_path_list=video_paths)
    
    ## inference
    with open(f'./test_longervideos/dataset.json', 'r') as f:
        longervideos = json.load(f)
    
    videorag = VideoRAG(llm=longervideos_llm_config, working_dir=f"./test_longervideos/videorag-workdir/{sub_category}")
    videorag.load_caption_model(debug=False)
    
    answer_folder = f'./test_longervideos/videorag-answers/{sub_category}'
