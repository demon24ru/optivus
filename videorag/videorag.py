...
class VideoRAG:
    ...
    def split_video(self, video_path_list=None):
        res = []
        for video_path in video_path_list:
            ...
            for index in segment_index2name:
                ...
                files.append({
                    'file': os.path.join(cache_path, f"{segment_name}.{self.audio_output_format}")
                })
    ...
    def insert_video(self, speech_to_text_store=[], video_path_list=None, split_video_store=None):
        ...
        for i, video_path in enumerate(video_path_list):
            ...
            # Step1: split the videos
            if split_video_store is not None:
                segment_index2name = split_video_store[i]['segment_index2name']
                segment_times_info = split_video_store[i]['segment_times_info']
            else:
                segment_index2name, segment_times_info = split_video(
                    video_path,
                    self.working_dir,
